...
const copyUrl = (event) => {
  ...
  navigator.clipboard.writeText(props.misub.url).then(() => {
    // 可以添加一个toast提示
  ...
  >
    ...
    <div class="mt-2 flex-grow flex flex-col justify-center space-y-2">
      <div class="relative">
        <div 
          @click="toggleUrl"
          class="w-full text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800/50 rounded-lg px-3 py-2 font-mono cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700/70 transition-colors flex items-center justify-between"
        >
          <span v-if="showUrl" class="truncate">{{ misub.url }}</span>
          <span v-else class="text-gray-400 dark:text-gray-500">点击显示链接</span>
          <div class="flex items-center gap-1 ml-2">
            <svg v-if="showUrl" class="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
            </svg>
        ...
        <button 
          ...
          @mouseup.stop
          @dragstart.stop
          class="absolute right-2 top-1/2 transform -translate-y-1/2 p-1 text-gray-400 hover:text-indigo-500 transition-colors"
          title="复制链接"
        >
