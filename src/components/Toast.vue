...
<template>
  <Transition name="toast">
    <div
      v-if="isVisible"
      class="fixed top-5 left-1/2 -translate-x-1/2 z-[100] px-6 py-4 rounded-2xl shadow-2xl text-white font-medium text-sm backdrop-blur-md border"
      :class="[
    ...
    >
      ...
      <div class="absolute bottom-0 left-0 right-0 h-1 bg-white/20 rounded-b-2xl overflow-hidden">
        <div class="h-full bg-white/60 rounded-b-2xl transition-all duration-3000 ease-linear" style="width: 100%"></div>
      </div>
    </div>
  </Transition>
</template>
