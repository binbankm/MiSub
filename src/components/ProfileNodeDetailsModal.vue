      ...
      if (subscription.url && subscription.url.startsWith('http')) {
        try {
          ...
          if (response.ok) {
            ...
            const parsedNodes = parseNodes(content, subscription.name);
...
// 解析节点信息
const parseNodes = (content, subscriptionName) => {
  const nodes = [];
  ...
  try {
    ...
    if (decodedLines.some(line => line.includes('://'))) {
      for (const line of decodedLines) {
        if (line.includes('://')) {
          const nodeInfo = parseNodeLine(line, subscriptionName);
          if (nodeInfo) {
            nodes.push(nodeInfo);
          }
        }
...
// 解析单行节点信息
const parseNodeLine = (line, subscriptionName) => {
  const nodeRegex = /^(ss|ssr|vmess|vless|trojan|hysteria2?|hy|hy2|tuic|anytls|socks5):\/\//;
  ...
  <div v-if="show" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[99] flex items-center justify-center p-4" @click="emit('update:show', false)">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-4xl text-left ring-1 ring-black/5 dark:ring-white/10 flex flex-col max-h-[85vh]" @click.stop>
      ...
      <div class="px-6 pb-6 flex-grow overflow-y-auto">
        <div class="space-y-4">
          <!-- 订阅组信息头部 -->
          <div v-if="profile" class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
